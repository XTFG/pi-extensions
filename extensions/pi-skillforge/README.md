...
```bash
just check              # biome check + all workspace typechecks
just format             # format files with Biome
just hooks              # install pre-commit hooks
just pack-skillforge    # preview npm package contents
just publish-skillforge # publish with --access public
just doctor @narumitw/pi-skillforge
```

## Release

Typical patch release from the repository root:

```bash
just bump @narumitw/pi-skillforge patch
just check
just publish-skillforge
```

For scoped npm packages, publishing must use public access. The `just publish-skillforge` recipe already runs:
