...
- Symptom: rejecting `/goal resume` for an unchanged exhausted budget can still invalidate the stale-turn guard. Cause: rotating the goal id before checking eligibility. Fix: reject an exhausted `budget_limited` goal before creating the next goal instance.
