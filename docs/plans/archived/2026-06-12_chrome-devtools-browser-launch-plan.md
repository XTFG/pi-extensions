...
- [x] Add a single `ensureDevToolsEndpoint()` helper used before both `/json/list` and `/json/new`; verified by code review that `listPages()` and `createPage()` pass through endpoint readiness and that `/json/new` uses `withEndpointRetry()` for transient startup failures.
