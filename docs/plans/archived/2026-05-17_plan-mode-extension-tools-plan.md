...
- [x] Persist the selected Plan-mode tools in the existing Plan mode state by tool name, while showing the current effective tool source from `sourceInfo`; verified by TypeScript checking and the selector smoke script preserving selected tool names.
