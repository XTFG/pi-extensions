...
- [x] **Stage 1 — Enforce mode invariants.** Added direct `tool_call` blocking for `update_plan` while preserving built-in mutation enforcement; hook tests pass in the 302-test root suite.
...
- [x] Repository quality gates pass with `npm run check` (302/302 tests), and extension loading is smoke-tested non-interactively with `pi -ne -e ./extensions/pi-plan-mode --help` showing `--plan`.
