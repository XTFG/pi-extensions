...
- [x] Extended persisted goal state with a backward-compatible active-start field, normalized older entries, and settled/restarted active duration on transitions; verified pause waiting, resume, edit, completion, shutdown/reload, and legacy entries with fake clocks.
...
- [x] Added a goal-scoped, single-flight custom wrap-up instruction, substantive-tool blocking, bounded completion permission, stale-context filtering, and failed-delivery retry; verified one accepted message across repeated events.
- [x] Made `/goal edit --tokens ...` reactivate a budget-limited goal only when the new budget exceeds current usage while preserving other stopped statuses; verified stale-goal-ID rotation.
...
- [x] Ran `npm run check` with 233 passing tests, the six-scenario real-AgentSession runtime smoke, checks against Pi 0.79.10 and 0.80.3, and `just pack-goal`; the package dry run contains only `LICENSE`, `README.md`, `package.json`, and `src/goal.ts`.
...
- [x] The model receives at most one non-completion wrap-up instruction, verified by repeated-event and failed-delivery tests.
