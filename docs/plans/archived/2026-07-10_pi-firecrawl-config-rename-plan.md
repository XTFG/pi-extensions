...
- [x] Overwriting `pi-firecrawl.json` when both files exist would lose the user's newer preference. Mitigated by rechecking the new path after asynchronous legacy reads and failed migration attempts, always preferring any concurrently created new file, and writing migrated legacy settings with `flag: "wx"` only when the new file is absent.
